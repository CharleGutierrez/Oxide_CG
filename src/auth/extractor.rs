...
async fn extract_user_from_parts(parts: &Parts, state: &AppState) -> Result<Option<AuthUser>, OxideError> {
    ...
    if let Some(cookie_header) = parts.headers.get("Cookie") {
        if let Ok(cookie_str) = cookie_header.to_str() {
            for pair in cookie_str.split(';') {
                ...
                if parts.len() == 2 && parts[0] == "oxide_session" {
