...
/// isolates the fault, returns a clean JSON 500, and allows the server to keep running.
...
fn handle_panic(err: Box<dyn Any + Send + 'static>) -> Response {
    ...
    error!("🛡️ [Self-Healing Panic Recovery] Intercepted panic in request handler: '{}'. Isolated successfully without process crash.", msg);

    let body = Json(json!({
        "success": false,
        "error": {
            "code": "PANIC_RECOVERED",
            "message": "An internal error occurred, but the server self-healed and isolated the failure.",
            "recovered": true
        }
    }));
