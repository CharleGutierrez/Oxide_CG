...
pub async fn rollback_handler(
    ...
    let row_opt = sqlx::query("SELECT model_name, record_id FROM _oxide_audit_logs WHERE id = ? LIMIT 1")
        .bind(log_id)
    ...
    let (model_name, record_id): (String, i64) = match row_opt {
        ...
        None => return Err(OxideError::NotFound("Audit log not found".to_string())),
    };

    let schema = state
        .registry
        .get(&model_name)
        .ok_or_else(|| OxideError::NotFound(format!("Schema for '{}' not found", model_name)))?;
    ...
    if success {
        ...
        Err(OxideError::Validation("Could not perform rollback on snapshot".to_string()))
    }
}
