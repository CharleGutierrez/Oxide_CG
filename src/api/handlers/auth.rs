    ...
    match state.auth_service.login(&payload.username, &payload.password, ip, ua).await? {
        Some(session) => {
            let cookie_header = format!(
                "oxide_session={}; Path=/; HttpOnly; SameSite=Lax; Max-Age=604800",
                session.token
            );
        ...
        None => Err(OxideError::Unauthorized("Invalid username or password".to_string())),
    }
}
...
pub async fn logout_handler(
    ...
    let cookie_header = "oxide_session=; Path=/; HttpOnly; Max-Age=0";
    let mut res_headers = HeaderMap::new();
    res_headers.insert("Set-Cookie", cookie_header.parse().unwrap());
...
pub async fn me_handler(
    ...
    match user_opt {
        ...
        None => Err(OxideError::Unauthorized("Not authenticated".to_string())),
    }
}
