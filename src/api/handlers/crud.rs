...
pub async fn get_record_handler(
    ...
    let record = state
        ...
        .get_by_id(schema, id)
        .await?
        .ok_or_else(|| OxideError::NotFound(format!("Record #{} not found in '{}'", id, model_name)))?;
...
pub async fn create_record_handler(
    ...
    let obj = match payload {
        ...
        _ => return Err(OxideError::Validation("Payload must be a JSON object".to_string())),
    };
...
pub async fn update_record_handler(
    ...
    let updated_record = state
        ...
        .await?
        .ok_or_else(|| OxideError::NotFound("Failed to update record".to_string()))?;
