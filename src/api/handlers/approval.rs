...
pub async fn approve_handler(
    ...
    let reviewer = match user {
        ...
        _ => return Err(OxideError::Forbidden("Requires Manager or Admin role".to_string())),
    };

    let row_opt = sqlx::query("SELECT model_name FROM _oxide_approvals WHERE id = ?")
        .bind(id)
    ...
    let model_name: String = match row_opt {
        ...
        None => return Err(OxideError::NotFound("Approval request not found".to_string())),
    };

    let schema = state
        .registry
        .get(&model_name)
        .ok_or_else(|| OxideError::NotFound(format!("Model '{}' not found", model_name)))?;
    ...
    if approved {
        ...
        Err(OxideError::Validation("Approval is no longer pending".to_string()))
    }
}
