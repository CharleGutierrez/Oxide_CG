...
impl AuditService {
    ...
    pub async fn list_logs(&self, limit: i64, offset: i64) -> Result<Vec<AuditLogEntry>, OxideError> {
        let rows = sqlx::query(
            r#"
            SELECT id, model_name, record_id, action, username, changes_json, snapshot_json, created_at
            FROM _oxide_audit_logs
            ORDER BY id DESC
            LIMIT ? OFFSET ?
            "#
        )
    ...
    pub async fn rollback(
        ...
        let row_opt = sqlx::query(
            "SELECT model_name, record_id, action, snapshot_json FROM _oxide_audit_logs WHERE id = ? LIMIT 1"
        )
        .bind(log_id)
        ...
        if let Some(row) = row_opt {
            ...
            if let Value::Object(obj) = snapshot {
                if action == "DELETE" {
                    ...
                    info!("Time-travel restored deleted record #{} in '{}'", record_id, schema.name);
                    return Ok(true);
                    ...
                    if !set_clauses.is_empty() {
                        ...
                        info!("Time-travel reverted record #{} in '{}'", record_id, schema.name);
                        return Ok(true);
                    }
                }
            }
