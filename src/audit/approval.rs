...
impl ApprovalService {
    ...
    pub async fn approve(
        ...
        let row_opt = sqlx::query(
            "SELECT model_name, record_id, field_name, new_value, status FROM _oxide_approvals WHERE id = ? LIMIT 1"
        )
        .bind(approval_id)
