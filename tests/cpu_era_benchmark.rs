...
/// Benchmark and verify Oxide_CG execution characteristics across 25 years of CPU architectures
#[test]
fn test_cpu_era_2000_to_2005_single_core_low_mem() {
    ...
    rt.block_on(async {
        let db_file = format!("/tmp/oxide_cpu_2000_{}.db", rand::random::<u64>());
...
fn test_cpu_era_2006_to_2011_quad_core_multi_threading() {
    ...
    rt.block_on(async {
        let db_file = format!("/tmp/oxide_cpu_2008_{}.db", rand::random::<u64>());
...
fn test_cpu_era_2012_to_2017_cloud_16_core_density() {
    ...
    rt.block_on(async {
        let db_file = format!("/tmp/oxide_cpu_2015_{}.db", rand::random::<u64>());
    ...
    rt.block_on(async {
        ...
        for worker_id in 0..64 {
            ...
            handles.push(tokio::spawn(async move {
                ...
                payload.insert("counter".to_string(), json!(worker_id));
                db_clone.insert(&schema_clone, &payload).await.unwrap()
            }));
        }
